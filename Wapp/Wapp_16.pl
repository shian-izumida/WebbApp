...
if($renewd ne ""){ &renewd; }
if($mode eq "" && $renewd eq "" && $post_data eq "" && $renew_post eq "" && $cat eq "" && $key eq ""){&main;}
if($renew_post){ &renew_post;} #更新時には、データ挿入を行う
&fooder();
...
sub post_data{
	...
	if($rec[0] > 0){
		...
		print qq|<a href="Wapp.pl?mode=main">back to top.</a>\n|;
	}else{
		my $sth = $dbh->prepare(
			"INSERT INTO course 
			VALUES(?,?,?,?,?,?,?)");
...
<a href="delete.pl?mode=$course_id">delete this course? ...</a>
<p>
<form action=Wapp.pl method="post">
<table border>
<tr><th>course_id</th>
...
sub renew_post{
	...
	$sth->bind_param(4,$level_id);
	$sth->bind_param(5,$category);
	$sth->bind_param(6,$course_id);
	if(!$sth->execute){
	print "SQL失敗\n";
	exit;
...
sub parse_form{
	...
	$renew_post = &sanitize($q->param("renew_post"));
	$renewd = $q->param('renewd');
	$key = &sanitize($q->param('key'));#keyword
	$cat = &sanitize($q->param('cat'));#category
	$renew_id = &sanitize($q->param('renew_id'));	
}
